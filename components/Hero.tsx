...
export function Hero({
  ...
  const showImage = Boolean(hero.image?.asset);
  ...
  return (
    <section className="relative overflow-hidden px-5 pb-16 pt-32 md:px-8 md:pb-24 md:pt-40">
      <div
        className={`mx-auto grid max-w-7xl items-center gap-14 ${
          showImage ? "lg:grid-cols-[1.05fr_0.95fr]" : "max-w-4xl justify-items-center text-center"
        }`}
      >
        <motion.div
          initial="hidden"
        ...
        >
          ...
          <motion.p
            variants={item}
            className={`mt-6 max-w-xl text-base leading-relaxed text-ink/75 md:text-lg ${showImage ? "" : "mx-auto"}`}
          >
            {hero.subhead}
          </motion.p>
          <motion.div
            variants={item}
            className={`mt-8 flex flex-col gap-3 sm:flex-row ${showImage ? "" : "sm:justify-center"}`}
          >
          ...
          <motion.ul
            variants={item}
            className={`mt-8 flex flex-wrap gap-x-6 gap-y-2 text-[13px] font-semibold text-ink/80 ${showImage ? "" : "justify-center"}`}
          >
